...
[ Физический щелчок мыши ]
                  │
                  ▼
 ФАЗА 1: [Прибор 10: Мышь] (Владелец такта)
         ├─ Читает сырые координаты из транзакта
         ├─ Мутирует регистр focusId в ОЗУ ядра хоста
         └─ Генерирует ВТОРИЧНЫЙ транзакт "MOUSE_CLICK" для Панели 102
                  │
                  ▼
 ФАЗА 2: [Прибор 102: Проводник] (Бизнес-редукция)
         ├─ Просыпается на этом же тике шины
         ├─ Забирает локальные координаты из пакета
         └─ Мутирует selectedIndex в своей модели данных в ОЗУ
                  │
                  ▼
 ФАЗА 3: [Пассивный Прибор 1: Рисовальщик] (Финальный барьер)
         ├─ Вызывается планировщиком шины ГАРАНТИРОВАННО ПОСЛЕДНИМ
         ├─ Ему не нужно ничего знать о мыши или кликах
         ├─ Он просто обходит пассивные ОЗУ-матрицы приборов
         └─ Снимает готовый слепок и выжигает дифф в ConPTY терминал

         Почему рисовальщику строго запрещено слать транзакты:Защита от бесконечной графической петли (Render Loop Crash):
         Если бы рисовальщик выстреливал транзакт (например, RENDER_DONE), 
         шина СМО была бы вынуждена снова запустить тактовый конвейер executeReactivePulsePipeline. 
         Тот прокрутил бы приборы, обнаружил изменения и в конце снова вызвал Канал 1. 
         Система ушла бы в мёртвое зацикливание со 100% нагрузкой на CPU, полностью парализовав обработку ввода.
         Пассивная природа Presentation-контура:
         Рисовальщик — это просто зеркало оперативной памяти. 
         Он не принимает бизнес-решений, не валидирует ввод и не меняет логическое состояние рантайма. 
         Он «слеп» к бизнес-логике: для него нет разницы между текстом лога или списком файлов — 
         он видит только мономорфные ячейки ОЗУ-матриц с полями char, fg и bg.


Ревизия спецификации: #0825-GEN-3-EXTENDED-DOD-MANIFЕSTБазовый уровень: Дополнение разделов 2, 3 и 4 на основе практической эксплуатации транзактной шины bus.js.🧩 1. ЗАКОН ТОТАЛЬНОЙ МОНОМОРФНОСТИ ВЬЮ-СТЕКОВДля предотвращения деоптимизации TurboFan и исключения полиморфизма дескрипторов при обходе UHD-матриц блайтером кадра tree_builder.js, вводится Единая DOD-форма памяти ViewStack:Абсолютный запрет на полиморфные типы: Свойство viewStack любого СМО-прибора обслуживания (как многовкладочных Проводников 102/103, так и плоских панелей 101, 105, 106, 108) обязан принудительно инициализироваться исключительно в виде плоского ОЗУ-массива фиксированной формы.Гвардирование плоских нод: Если узел в layout.json не содержит задекларированных пользовательских вкладок (например, CLI или Логгер), IoC-монтажник slot_maker.js обязан завернуть прибор в мономорфный массив из одной ячейки [0], где tabTitle принудительно заполняется апперкейс-именем самого компонента.Порядок запечатывания: При аллокации плоских вкладок сначала жестко запечатывается элемент массива, а затем — сам массив через Object.preventExtensions(). Нарушение последовательности сбрасывает массив в Dictionary Mode, приводя к потере указателей itemsList и исчезновению файлов.🎨 2. СУВЕРЕННЫЙ ПРИБОР ТРИАДЫ ВКЛАДОК (СЛОТ 12)В рамках декомпозиции Presentation-контура и очистки парсера мыши от бизнес-логики, управление каруселью верхних меню переведено под контроль выделенного инфраструктурного Канала 12:Изоляция PAC-Миксина: Слот 12 (tab_triad_mixin) функционирует как чистый функциональный PAC-композит. У него полностью отсутствует собственная модель данных (Model) и вид (View) — он оперирует чужой памятью viewStack активного прибора, идентификатор которого (targetSlotId) поставляется в контексте транзакта мыши.0% Динамических Импортов в Такте: На верхнем уровне блайтинга запрещено использование асинхронных вызовов await import() для отрисовки статических элементов. Переключение вкладок выполняется реактивным Контроллером Канала 12, а пассивный выжиг плашек возвращен во встроенный синхронный inline-режим вьюх с шагом сетки, жестко зафиксированным по спецификации 3.0.2-RELEASE (ширина плашки 4 знакоместа, шаг инкремента 5 знакомест с учетом разделителя │).Реактивный Импульс Перекраски: Контроллер Канала 12 является единственной авторизованной точкой мутации индекса activeStackIdx. При фиксации изменения он обязан:Выставить флаг глобальной грязи виртуального холста хоста kernel.virtualCanvasState.isDirty = true.Инвалидировать теневой буфер ConPTY через forceInvalidateShadowCanvas().Сгенерировать транзакт ADD_LOG_ENTRY на Канал 108 для вывода русскоязычного рапорта о перекраске.Форсировать продвижение RENDER-очереди (Канал 1).📥 3. СТРОГИЙ КОНТУР ИЗОЛЯЦИИ ТЕМ (СЛОТ 106)Вынос прикладных конфигураций палитр во внешний буфер подчинен закону Минимизации Избыточности Памяти (Data Bloat Starvation):Запрет дублирования: Базовым анемичным моделям слотов запрещено хранить массивы доступных ANSI-масок themesList.Локализация в Модели Компонента: Внешний файл конфигурации config/themes.json считывается асинхронным Лоадером (Канал 11) строго один раз при холодном бутстрапе. Массив палитр наливается строго внутрь ОЗУ-модели нулевого элемента в ViewStack Слота 106 (theme).Связывание через маркеры: Все сторонние интерактивные приборы (Проводники, Дашборд) хранят исключительно плоский числовой регистр _activeThemeId: 0. При попиксельном слиянии кадров декоратор frame_decorator.js осуществляет сквозное чтение ANSI-свойств выбранной строки темы из Слота 106, полностью сохраняя изоляцию слоев.🛡️ 4. ТРЕХЗОННЫЙ ФОКУС WINDOW MANAGER И FAULT TOLERANCEГлобальный потоковый парсер SGR-мыши Слота 10 переведен в режим пассивного геометрического WM-детектора:3 Потенциально Активные Зоны: Ввод разделен на жесткие интервалы без посимвольного сканирования растра:Зона 0 (Рамка / Периметр): localY === 0 || localY === h - 1 || localX === 0 || localX === w - 1. Генерирует интент RESIZE_SLOT_GEOMETRY на Канал 9. Любой скролл контента намертво блокируется.Зона 1 (Вкладки): localY === 1. Атомарно переводит колесико мыши в режим ротации карусели, транслируя WHEEL_UP/DOWN в интенты SCROLL_TABS_UP/DOWN для Канала 12.Зона 2 (Контент файлов / логов): localY >= 2 && localY < h - 1. Фиксирует фокус на теле прибора. Колесико транслируется в SCROLL_CONTENT_UP/DOWN, обеспечивая лавинный скролл строк.Санитарный щит Content Router: На стыке Presentation-слоев внедрен Fault Tolerance Buffer. Каждая ветка ESM-импорта изолирована локальной ловушкой try-catch (исключение из общего правила ядра для защиты ConPTY). При физическом отсутствии вью-компонента на диске, роутер блокирует падение V8 и автоматически выжигает в границах геометрии слота бордовую текстуру деоптимизации ░░░░ с центрированным HEX-паспортом краша [! FALLBACK SLOT X !] MODULE MISSING.
