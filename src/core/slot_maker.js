 ...
 * @version 15.2.0-RELEASE-SMO-IOC-CONVEYOR-STRICT-MONOMORPHIC-ARRAYS
 * @description Централизованный upper-level оркестратор сборки и монтажа абстрактных PAC-триад.
 * ИСПРАВЛЕНО: Полный перевод всех окон (включая одиночные) на честные массивы viewStack из 1 элемента.
...
export function executeDeferredSynchronization(kernel, payload) {
    ...
    if (isMultiTab === true) {
        ...
        const singleNameStr = Array.isArray(namesCacheArr) ? namesCacheArr[0] : (namesCacheArr || String(comp).toUpperCase());
        // ФИКС: Одиночный PAC-узел жестко укладываем в нулевую ячейку выделенного массива dynamicTabsStack
 ...
 * Время изменения: 10.09.2026 21:14:00 MSK
 */
